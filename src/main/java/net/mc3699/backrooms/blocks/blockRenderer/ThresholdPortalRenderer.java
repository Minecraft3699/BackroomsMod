...
import net.mc3699.backrooms.client.ModRenderTypes;
...
public class ThresholdPortalRenderer implements BlockEntityRenderer<ThresholdPortalBlockEntity> {
    ...
    {
        addVert(vertexConsumer,poseStack,0,0,0,0,0);
        addVert(vertexConsumer,poseStack,0,1,0,0,0);
        addVert(vertexConsumer,poseStack,1,1,0,0,0);
        addVert(vertexConsumer,poseStack,1,0,0,0,0);
    ...
    public void render(ThresholdPortalBlockEntity thresholdPortalBlockEntity, float v, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int i1) {
        ...
        VertexConsumer builder = multiBufferSource.getBuffer(ModRenderTypes.THRESHOLD_PORTAL);
        ResourceLocation testShader = ResourceLocation.fromNamespaceAndPath(BackroomsMod.MODID, "test_shader");

        ShaderProgram shader = VeilRenderSystem.setShader(testShader);
        if(shader == null)
        {
            return;
        }
